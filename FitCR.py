...
for quantity in ["Rhomass2Dunrolled"]: 
	...
	region = "CR" # we work in the sideband for now
	MC = ["Sig", "BkgDstarDs"] #"SigPart", "BkgDstarDsstar"
	...
	shapefile = ROOT.TFile.Open("fitDiagnosticsTest.root", "READ")
	bkgshape = shapefile.Get("shapes_fit_s/SB/bkg")
	if (not bkgshape): print("ERROR: No valid background shape in file!")
	regions = ["CR"]
	with open("datacardCR.txt", "w") as datacard: 
		...
		workspacefile = "workspaceCR.root"
		workspacename = "w"
		...
		#bkgNormCR = RooRealVar("bkg_CR_norm", "bkg_CR_norm", bins)
		#getattr(workspace, "import")(bkgNormCR)
		# Getting the bkg shape from the SB fit result 
		roohist = ROOT.RooDataHist("bkg_CR", "bkg_CR", fitspace, bkgshape)
		bkgshape.Write()
		getattr(workspace, "import")(roohist)
		...
		for region in regions: 
			for item in MC: 
				...
				expectedstring += "{} ".format(-1) #localnorm[item][region]
				count += 1
		...
		datacard.write("bkg_CR_norm rateParam CR bkg {} [{},{}]\n".format(norm["dataD2"]["CR"]/2., 0., norm["dataD2"]["CR"]))
		datacard.write("\n"+"-"*50+"\n")
